...
import ToastEditor from '@/components/editor/ToastModifyEditor.vue'
    ...
    export default {
      ...
      created(){
            this.travelAgencyId = this.$route.query.sn;
            this.dateInit();
            this.authChk();
            this.validTravel();
      },    
      methods: {
        ...
        this.$axios.get(process.env.VUE_APP_TRAVEL_AGENCY_LIST_CRUD+this.travelAgencyId,{headers}).then((res) =>{
         if(res.data.resultCode=="SUCCESS"){
            ...
            this.travelCity = data.city;
            this.personCount =data.person_max_count;
            this.startDate = data.travelRealTripAt.substr(0,4)+"-"+data.travelRealTripAt.substr(4,2)+"-"+data.travelRealTripAt.substr(6,2);
            this.realPay = data.real_paid;
            this.salePercent = data.sale_percent;
            this.salePay =  data.sale_paid;
            this.endDate =  data.travel_end_at.substr(0,4)+"-"+data.travel_end_at.substr(4,2)+"-"+data.travel_end_at.substr(6,2);
            this.agencyId = data.travel_agency_id;
            this.fileId = data.thumnbnailFileId;
            if(data.thumnbnailFileId!=null){
              this.preview = process.env.VUE_APP_FILE_IMAGE_READ+data.thumnbnailFileId+"/1";
            }
          }
        }).catch((error) => {
